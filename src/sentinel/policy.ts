...
// ── 1. Late fee waiver ────────────────────────────────────────────────
// Policy: max 1 waiver per 12 months, fee ≤ ₹1,000, ≥80% on-time record,
...
export async function checkLateFeeWaiver(customerId: number, feeId?: number): Promise<PolicyVerdict> {
  const POL = 'POL-002 · Goodwill fee waiver';
  ...
  if (c.cibil_score < 650) reason_codes.push('CIBIL_BELOW_650_CHRONIC_DELINQUENCY');
  if (c.card_status === 'closed') reason_codes.push('ACCOUNT_CLOSED');

  const eligible = reason_codes.length === 0 && missing_evidence.length === 0;
  ...
  return verdict('late_fee_waiver', POL, {
    ...
    required_next_step: eligible
      ...
      : missing_evidence.length
        ? 'Resolve missing evidence above before deciding.'
        : reason_codes.includes('FEE_EXCEEDS_AUTO_CEILING')
          ? 'Above the ₹1000 auto-waiver ceiling — route to a human via create_escalation; do not waive.'
          : reason_codes.includes('CIBIL_BELOW_650_CHRONIC_DELINQUENCY')
            ? `CIBIL score ${c.cibil_score} is critically low; goodwill waivers are reserved for customers demonstrating repayment discipline.`
...
export async function checkCreditLimitIncrease(customerId: number, requestedLimit?: number): Promise<PolicyVerdict> {
  ...
  const missed = history.filter((p) => p.payment_status === 'missed' || p.payment_status === 'late').length;
  ...
  if (vintageMonths == null) missing_evidence.push('card_issued_on');
  else if (vintageMonths < 6) reason_codes.push('VINTAGE_BELOW_6_MONTHS');
  if (c.cibil_score < 730) reason_codes.push('CIBIL_BELOW_730');
  ...
  if (c.card_status === 'closed') reason_codes.push('ACCOUNT_CLOSED');
  if (c.kyc_status === 'expired' || c.kyc_status === 'pending') reason_codes.push('KYC_NOT_CURRENT');
  ...
  return verdict('credit_limit_increase', POL, {
    ...
    required_next_step: eligible
      ...
      : reason_codes.includes('EXCEEDS_COMMITTEE_THRESHOLD_10L')
        ? `Requested limit exceeds ₹10,00,000 — requires Credit Committee approval. Create an escalation to Risk Operations.`
        : reason_codes.includes('EXCEEDS_AUTO_APPROVAL_CEILING')
          ? `Cap an auto-approval at ₹${maxAuto.toLocaleString('en-IN')}, or escalate for committee review above that.`
          : reason_codes.includes('UTILIZATION_ABOVE_90PCT_AFFORDABILITY_REVIEW')
            ? `Utilization is ${utilization}% (above 90%); this triggers an affordability review instead of auto-approval. Escalate to Risk Operations.`
            : reason_codes.includes('KYC_NOT_CURRENT')
              ? `KYC status is ${c.kyc_status}; limit increases require current KYC. Ask the customer to complete KYC re-verification first.`
...
export async function checkDuplicateRefund(customerId: number, transactionId?: string): Promise<PolicyVerdict> {
  ...
  if (!transactionId) missing_evidence.push('transaction_id (record_customer_transaction if not in account data)');
    ...
    // Check for recurring subscription pattern (same merchant, same amount, ~30 day interval)
    if (duplicate) {
      const t0Time = t0?.getTime() ?? 0;
      const dupTime = parseDate(duplicate.timestamp)?.getTime() ?? 0;
      ...
      if (recurring.length >= 3) {
        // If there are 3+ identical charges at this merchant, likely recurring
  ...
  return verdict('duplicate_refund', POL, {
    ...
    required_next_step: reason_codes.includes('POSSIBLE_RECURRING_SUBSCRIPTION_PATTERN')
      ? 'Multiple identical charges at this merchant suggest recurring billing, not a duplicate. Verify with the customer whether this is a subscription — check get_subscriptions.'
      : reason_codes.includes('BEYOND_60_DAY_REPORTING_WINDOW')
        ? 'This charge is older than 60 days from the statement date. Per policy, duplicate refunds must be reported within 60 days. The customer may raise a formal dispute instead.'
        : eligible
          ? `Confirmed duplicate of ${duplicate?.id}. Call initiate_refund on ONE charge (${transactionId}).`
          : reason_codes.includes('OPEN_DISPUTE_EXISTS')
            ? 'A dispute is already open — answer with get_disputes status instead of a new refund.'
            : reason_codes.includes('NO_DUPLICATE_PAIR_FOUND')
              ? 'No matching duplicate found (same merchant, same amount ±₹1, within 24 hours). Do NOT refund; offer raise_dispute if the customer still contests it.'
              : 'Resolve missing evidence / blocking reason before any refund.',
  });
}

// ── 4. E-mandate cancellation / opt-out ───────────────────────────────
export async function checkEmandateCancellation(customerId: number, subscriptionId?: string): Promise<PolicyVerdict> {
  ...
  return verdict('emandate_cancellation', POL, {
    eligible, reason_codes, facts_checked, missing_evidence,
    required_next_step: eligible
      ? `Call cancel_subscription for ${subscriptionId}. Note: this stops future debits only — past charges need initiate_refund / raise_dispute.`
...
export async function checkEmiConversion(customerId: number, transactionId?: string, tenureMonths?: number): Promise<PolicyVerdict> {
  ...
  if (c.minimum_due > 0 && c.due_date) {
    const dueDate = parseDate(c.due_date);
    if (dueDate && dueDate.getTime() < Date.now()) {
      reason_codes.push('OVERDUE_AMOUNT_EXISTS');
    }
  }
  if (c.cibil_score < 650) reason_codes.push('CIBIL_BELOW_650_ACCOUNT_STANDING');
  ...
  return verdict('emi_conversion', POL, {
    eligible, reason_codes, facts_checked, missing_evidence,
    required_next_step: eligible
      ? `Call convert_to_emi for ${transactionId}${tenureMonths ? ` at ${tenureMonths} months` : ' once the customer picks a tenure (3/6/9/12)'}. Processing fee: 1% of amount (min ₹199) + GST.`
      : reason_codes.includes('BEYOND_30_DAY_CONVERSION_WINDOW')
        ? 'EMI conversion must be requested within 30 days of the transaction date. This window has passed.'
        : reason_codes.includes('EMI_EXPOSURE_EXCEEDS_80PCT_OF_LIMIT')
          ? `Total EMI exposure would reach ${exposurePct}% of credit limit (max 80%). Customer must foreclose existing EMIs or pay down balance first.`
          : reason_codes.includes('OVERDUE_AMOUNT_EXISTS')
            ? 'Account has an overdue amount — EMI conversion requires the account to be current. Ask the customer to clear the overdue first.'
            : reason_codes.includes('EXCLUDED_CATEGORY')
              ? `Category "${target?.category}" is excluded from EMI conversion (fuel, cash advances, wallet loads, gold/jewellery).`
...
export async function checkFraudLiability(
  customerId: number,
...
// ── Tool wrappers ─────────────────────────────────────────────────────
export const checkLateFeeWaiverTool = defineTool({
...
export const checkCreditLimitIncreaseTool = defineTool({
...
export const checkDuplicateRefundTool = defineTool({
...
export const checkEmandateCancellationTool = defineTool({
...
export const checkEmiConversionTool = defineTool({
  ...
  parameters: Type.Object({
    customer_id: Type.Number(),
    transaction_id: Type.Optional(Type.String()),
    tenure_months: Type.Optional(Type.Number({ description: '3, 6, 9, or 12' })),
  }),
...
export const checkFraudLiabilityTool = defineTool({
